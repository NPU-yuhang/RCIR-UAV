...
void ObjectQualityInfo60C::unPackBytes(const can_msgs::Frame &can_frame)
{
    object_id_ = (uint8_t)(can_frame.data[0]);
    object_dist_long_rms_ = (uint8_t)((can_frame.data[1] & 0xf8) >> 3);
    object_dist_lat_rms_ = (uint8_t)((can_frame.data[1] & 0x07)<<2 + (can_frame.data[2] & 0xc0) >>6);
    object_vrel_long_rms_ = (uint8_t)((can_frame.data[2] & 0x3e) >> 1);
    object_vrel_lat_rms_ = (uint8_t)((can_frame.data[2] & 0x01)<< 4 + (can_frame.data[3] &0xf0) >> 4);
    object_arel_long_rms_ = (uint8_t)((can_frame.data[3]&0x0f) + (can_frame.data[4]&0x80)>>7);
    object_arel_lat_rms_ = (uint8_t)((can_frame.data[4]&0x7c)>>2);
    object_orientation_rms_ = (uint8_t)((can_frame.data[4]&0x03)<<3 + (can_frame.data[5]&0xe0)>>5);
    object_prob_of_exist_ = (uint8_t)((can_frame.data[6]&0xe0)>>5);
    object_meas_state_ = (uint8_t)((can_frame.data[6]&0x1c)>>2);
}
