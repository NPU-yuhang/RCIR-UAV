...
void ObjectGeneralInfo60B::unPackBytes(const can_msgs::Frame& can_frame)
{
    object_id_ = (uint8_t)(can_frame.data[0]);
    object_dist_long_ = ((uint16_t)(can_frame.data[1]<<5) + (uint8_t)((can_frame.data[2] & 0xf8)>>3)) * 0.2  - 500;
    object_dist_lat_ = ((uint16_t)((can_frame.data[2] & 0x07) << 8) + (uint8_t)(can_frame.data[3])) * 0.2 - 204.6;
    object_vre_long_ = ((uint16_t)(can_frame.data[4]<<2) + (uint8_t)((can_frame.data[5] & 0xc0)>>6)) * 0.25 - 128;
    object_vre_lat_ = ((uint16_t)((can_frame.data[5] & 0x3f) << 3) + (uint8_t)((can_frame.data[6]&0xe0)>>5)) * 0.25 - 64;
    object_dyn_prop_ = (uint8_t)(can_frame.data[6] & 0x07);
    object_rcs_ = (uint8_t)(can_frame.data[7]) * 0.5 -64.0;
}
