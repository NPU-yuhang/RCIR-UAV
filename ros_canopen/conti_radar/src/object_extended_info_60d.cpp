...
void ObjectExtendedInfo60D::unPackBytes(const can_msgs::Frame &can_frame)
{
    object_id_ = (uint8_t)(can_frame.data[0] & 0xff);
    object_arel_long_ = ((uint16_t)(can_frame.data[1] << 3) + (uint8_t)((can_frame.data[2]&0xe0)>>5)) * 0.01 -10;
    object_arel_lat_ = ((uint16_t)((can_frame.data[2]&0x1f)<<4) + (uint8_t)((can_frame.data[3]&0xf0)>>4)) * 0.01 - 2.5;
    object_class_ = (uint8_t)(can_frame.data[3] & 0x0f);
    object_orientation_angel_ = ((uint16_t)(can_frame.data[4]<<2) + (uint8_t)((can_frame.data[5]&0xc0)>>6))*0.4-180;
    object_length_ = ((uint8_t)(can_frame.data[6])) * 0.2;
    object_width_ = ((uint8_t)(can_frame.data[7])) * 0.2;
}
